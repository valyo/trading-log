...
Database: SQLite at `data/trades.db`. Created automatically on first run (including `price_series` / `price_bars` tables when the app starts).
...
```bash
docker compose -f compose.dev.yml exec app npm install
```

For a clean dependency layer, rebuild: `docker compose -f compose.dev.yml build --no-cache` then `up`.
